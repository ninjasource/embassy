...
/// OPSI hyperbus latency config.
#[derive(Clone, Copy)]
pub struct HyperbusLatencyConfig {
    /// True for fixed latency mode, otherwise variable latency mode
    pub fixed_latency_mode: bool,
    /// Enable zero latency on write operations
    pub write_zero_latency: bool,
    /// Device access time in number of communication clock cycles
    pub device_access_time_cycles: u8,
    /// Read write recovery time in number of communication clock cycles
    pub read_recovery_time_cycles: u8,
}

impl Default for HyperbusLatencyConfig {
    fn default() -> Self {
        Self {
            fixed_latency_mode: false,
            write_zero_latency: false,
            device_access_time_cycles: 0,
            read_recovery_time_cycles: 0,
        }
    }
}
...
pub struct Config {
    ...
    pub refresh: u32,
    /// Hyperbus latency config applicable to HyperBus memory_type
    pub hyperbus_latency_config: HyperbusLatencyConfig,
}
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            max_transfer: 0,
            refresh: 0,
            hyperbus_latency_config: Default::default(),
        }
    }
}
...
impl<'d, T: Instance, M: PeriMode> Ospi<'d, T, M> {
    ...
    fn new_inner(
        ...
        #[cfg(octospim_v1)]
        {
            ...
            // if the data strobe is used (for hyperbus) the io managed clock should not be turned on
            if dqs.is_none() {
        ...
        // set hyperbus latency config
        T::REGS.hlcr().modify(|w| {
            w.set_lm(match config.hyperbus_latency_config.fixed_latency_mode {
                true => vals::LatencyMode::FIXED,
                false => vals::LatencyMode::VARIABLE,
            });
            w.set_wzl(config.hyperbus_latency_config.write_zero_latency);
            w.set_tacc(config.hyperbus_latency_config.device_access_time_cycles);
            w.set_trwr(config.hyperbus_latency_config.read_recovery_time_cycles);
        });
...
impl<'d, T: Instance> Ospi<'d, T, Blocking> {
    ...
    /// Create new blocking OSPI driver using the HyperBus protocol (aka HyperRam) typically used for interfacing with external volatile memory
    /// The hyperbus should be ready to use in memory mapped mode after this call with no further action needed other than to keep the reference alive.
    pub fn new_hyperbus(
        peri: impl Peripheral<P = T> + 'd,
        sck: impl Peripheral<P = impl SckPin<T>> + 'd,
        d0: impl Peripheral<P = impl D0Pin<T>> + 'd,
        ...
        config: Config,
    ) -> Result<Self, OspiError> {
        let mut hyperbus = Self::new_inner(
            peri,
            new_pin!(d0, AfType::output(OutputType::PushPull, Speed::VeryHigh)),
            new_pin!(d1, AfType::output(OutputType::PushPull, Speed::VeryHigh)),
            new_pin!(d2, AfType::output(OutputType::PushPull, Speed::VeryHigh)),
        ...
        hyperbus.enable_memory_mapped_mode(read_config, write_config)?;
        Ok(hyperbus)
    }
}
